...
<head>
    ...
    <script type="text/javascript">
        $(function (){
            $("#seeDepartment").datagrid({
                title:'班级信息' ,
                url:'${pageContext.request.contextPath }/getDepartmentData?phone=${phone}',
                iconCls:'icon-save',
                width:'auto',
                height:'auto',
                // 自适应列宽
        ...
        function deleteUser(id) {
            $.post('${pageContext.request.contextPath }/deleteDepartment',{'departmentDO.id':id,phone:${phone}},function (data) {
                var d=eval('(data)');
                alert(d.message);
            });
        }
...
//
//            //打开新增的dialog
            var row = $('#seeDepartment').datagrid('getSelected');
            console.log(row);
            $('#dlg').dialog('open').dialog('setTitle','编辑');
            //将数据绑定到对应的数形值中
            $('#departmentDO.name').val(row.name);
            $('#departmentDO.operation').val(row.dname);
        }
        ...
        function addData() {
            ...
            //将数据data以json格式提交到后台
            changeStatus('${pageContext.request.contextPath }/addDepartment?phone=${phone}',data);
            $('#adddlg').dialog('close');
        }
        ...
        function saveData() {
            ...
            //console.log(data);类似于alert
            //将数据data以json格式提交到后台
            changeStatus('${pageContext.request.contextPath }/updateDepartment?phone=${phone}',data);
            $('#dlg').dialog('close');
        }
        ...
        function reloadTable(){
            $('#seeDepartment').datagrid('reload');
        }

    </script>
</head>
<body>
<table id="seeDepartment" class="easyui-datagrid" style="width: 100%;height:400px">
</table>
<div id="adddlg" class="easyui-dialog" style="width:330px;height:240px;padding:10px 20px">
    <form id="fm1" >
        ...
        <table style="width: 100%;">
            <tr>
                <td><span>名字</span></td>
                <td><input type="text" id="departmentDO1.name" name="name"/></td>
            </tr>
            <tr>
                <td><span>所属部门名称</span></td>
                <td><input type="text" id="departmentDO1.dname" name="dname"/></td>
            </tr>
...
<div id="dlg" class="easyui-dialog" style="width:330px;height:240px;padding:10px 20px"    >
    <form id="fm" >
        ...
        <table style="width: 100%;">
            <tr>
                <td><span>名字</span></td>
                <td><input type="text" id="departmentDO.name" name="name"/></td>
            </tr>
            <tr>
                <td><span>所属部门名称</span></td>
                <td><input type="text" id="departmentDO.dname" name="dname"/></td>
            </tr>
        </table>
        <div class="fitem" style="text-align: center">
